...
ggplot(simQSS,aes(QSS))+ geom_histogram(alpha=0.5,fill=col[1],color=col[1]) + theme_bw() + 
    ...
    geom_segment(data=tblQSS,aes(x=QSShigh,xend=QSShigh, y=20, yend = 0), colour=col[2], size=1, linetype=1 )


ggplot(simQSS,aes(Size, QSS,colour=Network))+ geom_point(alpha=0.5,shape = 21,fill="white") +  geom_point(data=websTbl[1,],aes(Size, QSS,colour=Network),shape = 21, size=2,fill="black") + theme_bw() +  scale_color_viridis_d() +  stat_ellipse(type="t") + coord_cartesian(ylim=c(0,0.8),expand = FALSE) 
...
ggplot(filter(simQSS, Network=="Weddell"),aes(Size, QSS,colour=Network))+ geom_point(alpha=0.5,shape = 21,fill="white") +  geom_point(data=websTbl[1:2,],aes(Size, QSS,colour=Network),shape = 21, size=2,fill="black") + theme_bw() +  scale_color_viridis_d() +  stat_ellipse(type="t") + coord_cartesian(ylim=c(0,0.8),expand = FALSE) 
