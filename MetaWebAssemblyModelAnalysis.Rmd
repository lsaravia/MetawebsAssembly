...
ig_meta <- graph_from_adjacency_matrix(metamat, mode="directed") 
...
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.015 - 0.005",se=0.25,model_type="Islands") )
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.25,e="0.15 - 0.05",model_type="Islands") )
...
m <- rep(0.015,times=dimA)
e <- rep(0.005, times=dimA)
se <- rep(0.55, times=dimA)
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.55,e="0.015 - 0.005",model_type="Islands"))
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.55,e="0.15 - 0.05",model_type="Islands"))
...
m <- rep(0.015,times=dimA)
e <- rep(0.005, times=dimA)
se <- rep(0.95, times=dimA)
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.015 - 0.005",se=0.95,model_type="Islands")) 
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.15 - 0.05",se=0.95,model_type="Islands"))

ggplot(AAdf, aes(S,C,color=factor(e)))+ geom_point() + theme_bw() + stat_ellipse() + geom_point( aes(mS,mC)) + facet_wrap( model_type ~ se) + scale_color_viridis_d(name="m - e" )




# Repeat for the Antarctic 
#
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.015 - 0.005",se=0.25,model_type="Antarctic") )
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.25,e="0.15 - 0.05",model_type="Antarctic") )
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.55,e="0.015 - 0.005",model_type="Antarctic"))
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),se=0.55,e="0.15 - 0.05",model_type="Antarctic"))
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.015 - 0.005",se=0.95,model_type="Antarctic")) 
...
AA <- metaWebNetAssemblyCT(A,m,e,se,tf)
plot_NetAssemblyModel_eqw(AA,100)
AAdf <- bind_rows( AAdf, tibble(C=AA$L[(tf-100):tf]/ (AA$S[(tf-100):tf])^2 ,S=AA$S[(tf-100):tf]) %>% mutate(mC = mean(C), mS=mean(S),e="0.15 - 0.05",se=0.95,model_type="Antarctic"))

ggplot(AAdf, aes(S,C,color=factor(e)))+ geom_point() + theme_bw() + stat_ellipse() + geom_point( aes(mS,mC)) + scale_color_viridis_d(name="c - e" ) +   facet_wrap( model_type ~ se , scales = "free" ) 

AAdf %>% group_by(model_type,e,se) %>% summarise(C = mean(mC),S=mean(mS))
ggsave("Figures/Metaweb_SecExtc_byModel.pdf",width=8,height=5,units="in",dpi=600)
...
```{r IslandSimulations150000tf1001, eval=FALSE,echo=F,message=T,warning=T}

metaweb_name <-  "Isl_Meta"

# Check redl is the metaweb
# 
stopifnot(  sum(degree(redl,mode="in")==0) == websTbl %>% filter(Network==metaweb_name) %>% dplyr::select(Basal) )
...
simMetaWebAssembly <- simMetaWebAssembly %>% filter(Metaweb!=metaweb_name)
# simulations with probability of secondary extinction == 1
#
# Discrete time
par_ranges <- matrix(c(0.01, 0.3,0.001, 0.5, 1, 1),nrow=3,byrow = TRUE)
...
metaweb_name <- "Isl"
fittedMetaWeb <- fittedMetaWeb %>% filter(Metaweb != metaweb_name)
...
#
# Correct Modularity with 100 evaluations
#
# websAssemblyModel <- websAssemblyModel %>% inner_join(websTbl %>% select(Network, Modularity), by=c("Network") )
# websAssemblyModel <- websAssemblyModel %>% relocate(Modularity, .after = SWnessCI)
...
```{r IslMetaWebAssemblyNew, eval=FALSE,echo=F,message=FALSE,warning=T}
...
# Correct WebsTbl
#
#websTbl <- websTbl %>% mutate(ismeta = if_else(grepl("_Meta",Network),TRUE,FALSE), Metaweb=if_else(is.na(Metaweb),"Isl",Metaweb)) 

websAssemblyModel <- websAssemblyModel %>% filter(!grepl("Isl_",Network)) 
websAssemblyModelSim <- websAssemblyModelSim %>% filter(!grepl("Isl_",Network)) 
websAssemblyModelSim %>% count(Network)
...
net_name <- pull( websTbl %>% filter(!ismeta) %>% dplyr::select(Network) )
model_name <- "Prob"
# websAssemblyModelSim <- websAssemblyModelSim %>% filter(model!="No SecExt")
...
if( !("C" %in% names(websAssemblyModelSim)) ) { 
  ...
  websAssemblyModel <- websAssemblyModelSim %>% group_by(model,Network) %>% group_modify( ~{ qq <- quantile(.x$C,c(0.005,0.995))
                                                                        ...
                                                                        }) %>% inner_join( websAssemblyModel, by=c("Network","model") )
  names(websAssemblyModel)
  websAssemblyModel <- websAssemblyModel %>% select(!(Clow.y:Shigh.y)) 
  names(websAssemblyModel)[3:6] <- c("Clow",  "Chigh", "Slow",  "Shigh")
...
websAssemblyModelSimQSS %>% count(Network,model)

websAssemblyModelQSS <- websAssemblyModelQSS %>% filter(!grepl("Isl_",Network)) 
websAssemblyModelSimQSS <- websAssemblyModelSimQSS %>% filter(!grepl("Isl_",Network)) 
...
if( FALSE ) { 
  websAssemblyModel <- websAssemblyModelSimQSS %>% group_by(model,Network) %>% group_modify( ~{ qq <- quantile(.x$QSS,c(0.005,0.995))
                                                                        ss <- quantile(.x$MEing,c(0.005,0.995))
                                                                        tibble(mdlQSS=mean(.x$QSS),QSSlow = qq[1],QSShigh=qq[2],mdlMEing=mean(.x$MEing),
                                                                               MEingLow=ss[1],MEingHigh=ss[2])
                                                                        }) %>% inner_join( websAssemblyModel, by=c("Network","model") )
  names(websAssemblyModel)
  websAssemblyModel <- websAssemblyModel %>% select(-(mdlMEing.y:MEingHigh.y), -(mdlQSS.y:QSShigh.y)) 
  names(websAssemblyModel)[3:8] <- c("mdlQSS",    "QSSlow",    "QSShigh",   "mdlMEing",  "MEingLow",  "MEingHigh")
  websAssemblyModel <- websAssemblyModel %>% select(-QSS,-MEing) %>% inner_join(websTbl %>% select(Network,QSS,MEing), by=c("Network"))  
...
knitr::kable(websAssemblyModel %>% inner_join(websTbl %>% dplyr::select(Network,Q)) %>% select(Size, Slow,Shigh,Connectance,Clow,Chigh,TLmean,TIlow,TIhigh), digits = 4)

knitr::kable(websAssemblyModel %>% inner_join(websTbl %>% dplyr::select(Network,Q)) %>% ungroup() %>% mutate(Metaweb=str_sub(Network,1,3),Network=str_sub(Network,5)) %>% select(Metaweb,Network,Q,Qlow,Qhigh,Modularity,MOlow,MOhigh,MEing,MEingLow,MEingHigh), digits = 4)
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size=7))  + coord_flip()
# ggsave("Figures/MEing_Assembly_byMeta.png",width=8,height=6,units="in",dpi=600)

websAssemblyModelQSS %>% mutate(his=MEingHigh,los=MEingLow,vs=MEing,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths, metric="MEing") %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Deviation")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75)) +ylim(-.8,.8) + facet_wrap(~metric)

ggsave("Figures/MEing_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)
...
# Trophic Coherence 
#
websAssemblyModel %>% inner_join(websTbl %>% dplyr::select(Network,Q))  %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% str_replace_all('_',' ' )) %>% mutate(his=Qhigh,los=Qlow,vs=Q,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Q")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75)) +ylim(-.8,.8) 
ggsave("Figures/Q_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)
...
websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% 
                               str_replace_all('_',' ' )) %>% mutate(his=MOhigh,los=MOlow,vs=Modularity,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Modularity")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75)) +ylim(-.8,.8) 
ggsave("Figures/Modularity_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)
...
websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% 
                               str_replace_all('_',' ' )) %>% mutate(his=TIhigh,los=TIlow,vs=TLmean,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Mean Trophic Level")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-1,-.5,0,.5,1)) #+ylim(-.8,.8) 
ggsave("Figures/TL_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)
...
websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% str_replace_all('_',' ' )) %>% mutate(his=Chigh,los=Clow,vs=Connectance,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Connectance")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75)) #+ylim(-.8,.8) 
ggsave("Figures/Connectance_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)
...
websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% str_replace_all('_',' ' )) %>% mutate(his=Shigh,los=Slow,vs=Size,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Size")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75)) #+ylim(-.8,.8) 

ggsave("Figures/Size_Assembly_byMeta.pdf",width=5,height=8,units="in",dpi=600)


#
# Combined Trophic Coherence and MEing  
#

pFM1 <- websAssemblyModel %>% inner_join(websTbl %>% dplyr::select(Network,Q))  %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% str_replace_all('_',' ' )) %>% mutate(his=Qhigh,los=Qlow,vs=Q,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths,metric="Trophic Coherence")  %>% select(Metaweb,Net_name,metric,vxs)
pFM2 <-websAssemblyModelQSS %>% mutate(his=MEingHigh,los=MEingLow,vs=MEing,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths, metric="MEing") 

bind_rows(pFM1,pFM2) %>% ungroup() %>% mutate(metric=fct_inorder(metric) ) %>% ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Deviation")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-.75,-.5,0,.5,.75))  + facet_wrap(~metric)

ggsave("Figures/MEing_Q_Assembly_byMeta.pdf",width=5.5,height=7,units="in",dpi=600)



#
# Combined Trophic Level and Modularity 
#
pFM1 <- websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% 
                               str_replace_all('_',' ' )) %>% mutate(his=MOhigh,los=MOlow,vs=Modularity,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths, metric="Modularity") %>% select(Metaweb,Net_name,metric,vxs) 
pFM2 <- websAssemblyModel %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% 
                               str_replace_all('_',' ' )) %>% mutate(his=TIhigh,los=TIlow,vs=TLmean,
                                widths = his-los, 
                                mids = los + (his-los)/2,
                                vxs = (vs - mids) / widths,metric="Trophic Level")  %>% select(Metaweb,Net_name,metric,vxs)

bind_rows(pFM1,pFM2) %>% ungroup() %>% group_by(metric) %>% ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Deviation")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-1,-.5,0,.5,1))  + facet_wrap(~metric)

ggsave("Figures/Modularity_TL_Assembly_byMeta.pdf",width=5.5,height=7,units="in",dpi=600)
...
```{r Isl_Motif_MetaWebAssembly, eval=FALSE,echo=F,message=FALSE,warning=T}

#if(!exists("websAssemblyModelQSS")) 
#motif_AssemblyModel <- tibble()
motif_AssemblyModel <- motif_AssemblyModel %>% filter(!grepl("Isl_",Network)) 
motif_AssemblyModel %>% count(Network)
...
lapply(net_name, function(fw){ 
    ...
    tblMdlWeb <- tbl %>% mutate(Network=fw,model=model_name) %>% dplyr::select(Network, model, everything())
...
knitr::kable(motif_AssemblyModel %>%  mutate(Metaweb=str_sub(Network,1,3),Network=str_sub(Network,5)) %>% select(Metaweb,Network,explComp:omnivory,EClow:OMhigh),digits=0)
...
pFM1 <- motif_AssemblyModel %>% rename(ECemp=explComp,ACemp=apprComp,TTemp=triTroph,OMemp=omnivory) %>%  select(!c(model,zEC:zOM))%>% pivot_longer(cols = c(ECemp:OMhigh) ,names_to=c("motif",".value"),names_pattern = "(EC|AC|TT|OM)\\s*(\\w+)") 

pFM1 <- pFM1 %>% mutate(motif=factor(motif),motif=fct_recode(motif, "exploitative competition" = "EC","apparent competition" = "AC", "tri-trophic chain" = "TT","omnivory"= "OM"))

pFM1 %>% mutate(Metaweb=str_sub(Network,1,3),Net_name = gsub('([a-z])([A-Z])', '\\1 \\2', str_sub(Network,5)) %>% str_replace_all('_',' ' )) %>% 
                                mutate(  widths = high-low, 
                                mids = low + (high-low)/2,
                                vxs = (emp - mids) / widths) %>% 
  ggplot( aes(x=fct_reorder(Net_name,vxs),y=vxs,color=Metaweb)) + geom_point(size=2) + 
  scale_color_brewer(palette = "Paired")+ theme_bw() + ylab("Deviation")  + xlab(NULL) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.justification=c(1,0), legend.position=c(.99,.01) ) + coord_flip() + 
  geom_hline(yintercept = 0,size=.3) + geom_hline(yintercept = 0.5,linetype=3) + geom_hline(yintercept = -0.5,linetype=3) + 
  scale_y_continuous(breaks = c(-1,-.5,0,.5,1)) + facet_wrap( ~motif, ncol=4 )#+ylim(-.8,.8) 
  
ggsave("Figures/Motif_Zscores_Assembly.pdf",width=8,height=7,units="in",dpi=600)
...
```{r MetaWeb_TopologicalRoles, eval=TRUE,echo=F,message=F,warning=T}
if(!exists("topoRoles_metaWebAssembly")) {
  topoRoles_metaWebAssembly <- tibble()
  hub_conn_Assembly <- tibble()
} else {
  topoRoles_metaWebAssembly <- topoRoles_metaWebAssembly %>%  mutate(rep=if_else(is.na(rep),0,rep))
  hub_conn_Assembly <- hub_conn_Assembly %>% mutate(rep=if_else(is.na(rep),0,rep))
  #topoRoles_metaWebAssembly <- topoRoles_metaWebAssembly %>%  filter(rep!=2)
  #hub_conn_Assembly <- hub_conn_Assembly %>% filter(rep!=2)
  topoRoles_metaWebAssembly %>% distinct(rep)
}
#set.seed(4334)
model_name <- "Prob"
...
# Re-run Islands
#
topoRoles_metaWebAssembly <- topoRoles_metaWebAssembly %>% filter(!grepl("Isl_",Network)) 
hub_conn_Assembly <- hub_conn_Assembly %>% filter(!grepl("Isl_",Network)) 
topoRoles_metaWebAssembly %>% count(Network)
hub_conn_Assembly  %>% count(Network)
...
foreach( rr=0:21 ) %do%
{
  ...
  lapply(net_name, function(fw){
      ...
      message(paste("\ncalc_topoRoles_metaWebAssembly", fw, model_name,rr))
      ...
      tictoc::toc()
      topoRoles_metaWebAssembly <<- topoRoles_metaWebAssembly %>% bind_rows(tbl$tr %>% mutate(model=model_name,rep=rr))
      hub_conn_Assembly <<-hub_conn_Assembly %>% bind_rows(tbl$hc %>% mutate(model=model_name,rep=rr))
  })
...
#
#load(".RData")
hub_conn_Assembly <- hub_conn_Assembly %>% filter(!rep  %in% c(13,16))
#topoRoles_metaWebAssembly <- topoRoles_metaWebAssembly %>% filter(!rep  %in% c(13,16))
knitr::kable(hub_conn_Assembly %>% count(rep))

topoRoles_metaWebAssembly %>% distinct(Network)
hub_conn_Assembly %>% distinct(Network) 
# Antarctic
#
...
#
topoRoles_metaWebAssembly_chi2 %>% distinct(Network)
topoRoles_metaWebAssembly_chi2 <- topoRoles_metaWebAssembly_chi2 %>% filter(!grepl("Ant_",Network))
                                         
foreach( rr=unique(hub_conn_Assembly$rep) ) %do% {
  
  hc <- hub_conn %>% filter(grepl("Ant",Network)) %>% bind_rows(
    hub_conn_Assembly %>% filter(grepl("Ant",Network),rep==rr) %>% dplyr::select(type,node,Network) %>% mutate(model="Assembly")) %>% 
    mutate(model=ifelse(is.na(model),"Empirical",model)) %>% group_by(Network,model,type) %>% summarise(count=n()) %>% mutate(total=sum(count),Freq=count/total) %>% inner_join(websTbl %>% dplyr::select(Network,Size))
  ...
  tt1 <- hc %>% group_by(Network) %>% group_modify( ~{ 
    ...
    }) %>% arrange(desc(Size)) %>% mutate(meta="Ant", rep=rr)
  
  topoRoles_metaWebAssembly_chi2 <- bind_rows(topoRoles_metaWebAssembly_chi2,tt1)
}
tt <- topoRoles_metaWebAssembly_chi2 %>% filter(meta=="Ant",rep==rr) %>% group_by(meta,rep) %>% slice()

#
# Plot
#

hc <- inner_join(hc, tt %>% dplyr::select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),as.character(Network)),as.character(Network)))
...
topoRoles_metaWebAssembly_chi2 <- topoRoles_metaWebAssembly_chi2 %>% filter(meta != "Isl")
...
foreach( rr=unique(hub_conn_Assembly$rep) ) %do% {
  ...
  hc <- hub_conn %>% filter(grepl("Isl",Network)) %>% bind_rows(
    hub_conn_Assembly %>% filter(grepl("Isl",Network),rep==rr) %>% dplyr::select(type,node,Network) %>% mutate(model="Assembly")) %>% 
    mutate(model=ifelse(is.na(model),"Empirical",model)) %>% group_by(Network,model,type) %>% summarise(count=n()) %>% 
    mutate(total=sum(count),Freq=count/total) %>% inner_join(websTbl %>% dplyr::select(Network,Size))
  
  hc <-  hc %>% ungroup() %>%  mutate(Network = ifelse(str_length(Network) <8, str_sub(Network,5), paste0(str_sub(Network,5),"web"))) %>% mutate(Network = fct_reorder2(Network, -Size, Network)) 
  ...
  tt0 <- hc %>% group_by(Network) %>% group_modify( ~{ 
    ...
    #print(ct)
    tibble(Size=unique(.x$Size),chi2=cc$statistic,pvalue = cc$p.value,method=cc$method)
    }) %>% arrange(desc(Size)) %>% mutate(meta="Isl", rep=rr)

  topoRoles_metaWebAssembly_chi2 <- bind_rows(topoRoles_metaWebAssembly_chi2,tt0)
}
tt <- topoRoles_metaWebAssembly_chi2 %>% filter(meta=="Isl")

#
# Plot
# 
hc <- inner_join(hc, tt0 %>% dplyr::select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),as.character(Network)),as.character(Network)))
t <- hc %>% filter(Network=="Metaweb")
t[1,3] <- "hubcon"
t[1,4] <- 0
t[1,6] <- 0
hc <- bind_rows(hc,t[1,])
...
(gP3 <- ggplot(hc,aes(x=model,y=Freq,fill=type)) + 
  geom_bar(stat="identity", position=position_dodge()) + 
  scale_fill_brewer(palette = "Paired",name="") + 
  xlab("") + ylab("") + theme_bw() + 
    theme(legend.position="bottom", legend.direction="horizontal",   strip.background = element_blank(), strip.placement = "outside") + 
...
ggsave("Figures/Isl_TopoRoles_ByNetwork_Model.pdf",width=8,height=5,units="in",dpi=600)

tt0 %>% summarize(sign = pvalue<0.01) %>% filter(sign) 
#topoRoles_metaWebAssembly_chi2 <- bind_rows(tt1,tt0)

unique(topoRoles_metaWebAssembly_chi2$Network)
```
...
```{r Lak_plotsMetaWebTopologicalRoles, eval=FALSE,echo=F,message=T,warning=T}
require(tidyverse)
# Lakes
#
# Comparison of model with empirical  
...
hc <- hub_conn %>% filter(grepl("Lak",Network)) %>% bind_rows(
  hub_conn_Assembly %>% filter(grepl("Lak",Network),rep==rr) %>% select(type,node,Network) %>% mutate(model="Assembly")) %>% 
...
tt <- hc %>% group_by(Network) %>% group_modify( ~{ 
  ...
  #print(ct)
  tibble(Size=unique(.x$Size),chi2=cc$statistic,pvalue = cc$p.value,method=cc$method)
  }) %>% arrange(desc(Size))  %>% mutate(meta="Lak", rep=rr)

  topoRoles_metaWebAssembly_chi2 <- bind_rows(topoRoles_metaWebAssembly_chi2,tt)

}
tt <- topoRoles_metaWebAssembly_chi2 %>% filter(meta=="Lak") %>% group_by(Network,rep) %>% count()

#
# Summary of results percent of significant results at p=0.01 
#
hc <- topoRoles_metaWebAssembly_chi2 %>% filter(Network!="Metaweb") %>% mutate(sign= if_else(pvalue<0.01, TRUE,FALSE)) %>%
  group_by(rep,sign)  %>% summarise(n=n()) %>% mutate(total=sum(n),Freq=n/total) %>% filter(sign==TRUE)
...
#
hc <- inner_join(hc, tt %>% dplyr::select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),Network),Network)) %>% mutate(net_name=if_else(str_length(net_name)>18,str_sub(net_name,1,-6),net_name))


(gP4 <- ggplot(hc,aes(x=model,y=Freq,fill=type)) + 
  geom_bar(stat="identity", position=position_dodge()) + 
  scale_fill_brewer(palette = "Paired",name="") + 
  xlab("") + ylab("") + theme_bw() + 
...
knitr::kable(topoRoles_metaWebAssembly_chi2 %>% filter(rep==21,Network!="Metaweb") %>% mutate(Network=if_else(pvalue<0.01, paste0(Network, ' *'),Network)) %>% dplyr::select( Network,meta,Size, chi2,pvalue),digits = 4)
topoRoles_metaWebAssembly_chi2 %>% filter(rep==21,!grepl("Meta",Network)) %>% summarize(sign = pvalue<0.05) %>% filter(sign, ) 


#
# Combined Antarctic Island Plot 
#
require(cowplot)
gP5 <- plot_grid(gP2 +  theme(legend.position="none"),
                 gP3 +  theme(legend.position="none"),
                 align = "vh",
                  hjust = -1,
                  nrow = 1, labels = c("A","B"))
...
plot_grid(gP5, legend_b, ncol = 1, rel_heights = c(1, .1))

ggsave("Figures/Ant_Isl_TopoRoles_ByNetwork_Model.png",width=9,height=6,dpi=600)


#
# Check different repetitions 
#
da <- topoRoles_metaWebAssembly %>% filter(grepl("Isl",Network)) %>% select(within_module_degree,among_module_conn,rep) %>% mutate(rep=factor(rep))
ggplot(da,aes(within_module_degree, color=rep,fill=rep)) + geom_density(alpha=0.5) + scale_fill_viridis_d() + scale_color_viridis_d()
ggplot(da,aes(among_module_conn, color=rep,fill=rep)) + geom_density(alpha=0.5) + scale_fill_viridis_d() + scale_color_viridis_d()

ad.test(within_module_degree ~ rep,data=da)
ad.test(among_module_conn ~ rep,data=da)

hub_conn_Assembly %>% count(Network,rep,type) 
```

* Most are not different from metaweb assembly model. Only 5 are significative at 1%
