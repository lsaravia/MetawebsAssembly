...
#
#
names(hub_conn_Assembly)

hc <- hub_conn %>% filter(Network=="Ant_Meta") %>% bind_rows(hub_conn_Assembly %>% filter(grepl("Ant",Network)) %>% select(type,node,Network) )
tbl1 <- table(hc$Network,hc$type)
...
# 
levels(df1$Network) <- c("Metaweb","Potter Cove\nModel","Weddell Sea\nModel")
...
#
hc <- hub_conn %>% filter(grepl("Ant",Network))
tbl <- table(hc$Network,hc$type)
...
levels(df2$Network) <- c("Metaweb","Potter Cove","Weddell Sea")
...
save_plot("Figures/Ant_TopoRoles_ByNetwork_Model.png",gP3,base_width=8,base_height=5,dpi=600)
...
tt %>% summarize(sign = pvalue<0.05) %>% filter(sign) 
...
* Most are not different from metaweb assembly model. Only 5 are significative at 5%