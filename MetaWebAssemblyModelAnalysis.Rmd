...
```{r makeSimulations50002, eval=TRUE,echo=F,message=T,warning=T}
...
require(lhs)
X <- randomLHS(50000, 3)
m <- qunif(X[,1], 0.01, 0.3)
e <- qunif(X[,2], 0.001, 0.5)
se <- qunif(X[,2], 0.1, .9)
...
tic()
sim <- foreach(i=1:nsim,.combine='rbind',.inorder=FALSE,.packages='meweasmo') %dopar% 
{
...
ggplot(simMetaWebAssembly, aes(S,C))+ geom_point(size=0.1,alpha=0.5) + theme_bw() + geom_point(data=websTbl, aes(Size,Connectance,color=Network),size=0.3) + coord_cartesian(ylim=c(0,0.1))

ggplot(simMetaWebAssembly, aes(S,L))+ geom_point(size=0.1,alpha=0.5) + theme_bw() + geom_point(data=websTbl, aes(Size,Links,color=Network),size=0.3) + coord_cartesian(ylim=c(0,3000))

ggplot(simMetaWebAssembly %>% mutate(secExt = se<1 ), aes(S,L,color=secExt))+ geom_point(size=0.1,alpha=0.5) + theme_bw() + geom_point(data=websTbl, aes(Size,Links,color=Network),size=0.3) + coord_cartesian(ylim=c(0,9100)) + scale_color_brewer(palette= "Paired",name="", labels=c("Fixed Sec.Ext.", "Potter","Meta","Weddell", "Prob. Sec.Ext." ))  
...
```{r fitMetaWebPC50002, eval=TRUE,echo=F,message=T,warning=T}
...
tol <- .01
sel <- simMetaWebAssembly %>% mutate(alpha=m/a) %>% filter(S>websTbl$Size[1]*(1-tol),S<websTbl$Size[1]*(1+tol), se<1) %>% arrange(S)
#
