...
```{r readAntarctic, eval=TRUE,echo=F,message=T,warning=T}
#
...
```{r makeSimulations150000tf1000, eval=TRUE,echo=F,message=T,warning=T}
...
knitr::kable(fittedMetaWeb %>% dplyr::select( Metaweb,Network,  everything(), -min_dist, -model_type) %>% arrange(Network), digits = 4)
...
# 
fittedMetaWeb_gof %>% filter(gof_pvalue>0.1) %>% count(model)

knitr::kable(fittedMetaWeb %>% ungroup() %>% dplyr::select( Metaweb,Network,  everything() )%>% select(-min_dist, -model_type,-fit_type) %>% arrange(Network), digits = 4)
...
```{r AntarcticMetaWebAssembly, eval=FALSE,echo=F,message=T,warning=T}
if(!exists("websAssemblyModel")) websAssemblyModel <- tibble()
if(!exists("websAssemblyModelSim")) websAssemblyModelSim <- tibble()
