...
```{r AntQSSMetaWebAssembly, eval=TRUE,echo=F,message=FALSE,warning=T}
...
lapply(net_name, function(fw){ 
    ...
    websAssemblyModelQSS <<- websAssemblyModel %>% bind_rows(tblMdlWeb) 
    websAssemblyModelSimQSS <<-websAssemblyModelSim %>% bind_rows(sim %>% mutate(Network=fw,model=model_name))  
})
save.image()
