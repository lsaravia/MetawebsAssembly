...
```{r readLakes, eval=TRUE,echo=TRUE,message=FALSE }
...
stopifnot(sum(degree(redl,mode="in")==0) == websTbl %>% filter(Network=="Lak_Meta") %>% select(Basal))
...
```{r LakesSimulations150000tf1000, eval=TRUE,echo=F,message=T,warning=T}
