...
```{r IslMetaWebAssemblyNew, eval=TRUE,echo=F,message=FALSE,warning=T}
...
```{r IslQSSMetaWebAssembly, eval=TRUE,echo=F,message=FALSE,warning=T}
...
```{r MetaWebTopologicalRoles, eval=FALSE,echo=F,message=F,warning=T}
...
topoRoles_metaWebAssembly %>% count(rep)
hub_conn_Assembly %>% count(rep) 
# Antarctic
#
...
#
topoRoles_metaWebAssembly_chi2 <- tibble()
foreach( rr=unique(hub_conn_Assembly$rep) ) %do% {
  
  hc <- hub_conn %>% filter(grepl("Ant",Network)) %>% bind_rows(
    hub_conn_Assembly %>% filter(grepl("Ant",Network),rep==rr) %>% select(type,node,Network) %>% mutate(model="Assembly")) %>% 
...
tt <- topoRoles_metaWebAssembly_chi2 %>% filter(meta=="Ant") %>% group_by(meta,rep) %>% slice()

#
# Plot
#

hc <- inner_join(hc, tt1 %>% select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),as.character(Network)),as.character(Network)))
...
foreach( rr=unique(hub_conn_Assembly$rep) ) %do% {
  ...
  hc <- hub_conn %>% filter(grepl("Isl",Network)) %>% bind_rows(
    hub_conn_Assembly %>% filter(grepl("Isl",Network),rep==rr) %>% select(type,node,Network) %>% mutate(model="Assembly")) %>% 
    mutate(model=ifelse(is.na(model),"Empirical",model)) %>% group_by(Network,model,type) %>% summarise(count=n()) %>% 
    mutate(total=sum(count),Freq=count/total) %>% inner_join(websTbl %>% select(Network,Size))
...
# Plot
# 
hc <- inner_join(hc, tt0 %>% select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),as.character(Network)),as.character(Network)))
...
#
hc <- topoRoles_metaWebAssembly_chi2 %>% mutate(sign= if_else(pvalue<0.01, TRUE,FALSE)) %>%
  group_by(meta,Network,sign)  %>% summarise(n=n()) %>% mutate(total=sum(n),Freq=n/total) %>% filter(sign==TRUE,Network!="Metaweb")
...
#
hc <- inner_join(hc, tt %>% select(Network,pvalue)) %>% mutate(net_name=if_else(pvalue<0.01, if_else(!grepl("Meta",Network), paste0(Network, ' *'),Network),Network)) %>% mutate(net_name=if_else(str_length(net_name)>18,str_sub(net_name,1,-6),net_name))
