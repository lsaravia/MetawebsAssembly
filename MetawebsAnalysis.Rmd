...
hc <- hub_conn %>% mutate(type = factor(type)) %>% filter(Network=="Local") %>% arrange(node) %>% mutate(col= as.numeric(type), TL=troph.net2[,1]) 
...
maxold <- 1
minold <- -1
t1 <- function(x) (maxnew -minnew)/(maxold-minold)*(x - maxold)+maxnew 
...
png("Figures/LocalTR_TL_MOD.png",width=6,height=6,units="in",res=600)
...
legend("topright", pch=19, col=colnet, legend= legstr)
...
hc <- hub_conn %>% mutate(type = factor(type)) %>% filter(Network=="Regional") %>% arrange(node) %>% mutate(col= as.numeric(type), TL=troph.net2[,1]) 
V(redl)$color <- colnet[hc$col]
...
png("Figures/RegionalTR_TL_MOD.png",width=6,height=6,units="in",res=600)
par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(.5, 3, .5, 0.5))
...
legstr <- levels(hc$type)
legstr <- c("Hub conn.", "Mod. Conn.", "Mod. Hubs", "Mod. Spec.")
legend("topleft", pch=19, col=colnet, legend= legstr)
...
hc <- hub_conn %>% mutate(type = factor(type)) %>% filter(Network=="Meta") %>% arrange(node) %>% mutate(col= as.numeric(type), TL=troph.net2[,1]) 
V(redl)$color <- colnet[hc$col]
...
png("Figures/MetawebTR_TL_MOD.png",width=6,height=6,units="in",res=600)
par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(.5, 3, .5, 0.5))
