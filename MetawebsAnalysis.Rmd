...
degree_distribution(redl)
websTbl <- calc_topological_indices(redl) %>% mutate(Network="Local",Groups=max(modulos$membership),Modularity=modulos$modularity)
...
modulos<-cluster_spinglass(redl)
websTbl <- bind_rows(websTbl, data.frame(calc_topological_indices(redl) %>% mutate(Network="Regional",Groups=max(modulos$membership),Modularity=modulos$modularity)))
#
# Make short names to check duplicates!!!!!!!!!! 
#
require(vegan)
spnames <- make.cepnames(V(redl)$name)

matrix(c(V(redl)$name[grep("Euchanta",spnames)], # Sinonims
V(redl)$name[grep("Notoanta",spnames)],
V(redl)$name[grep("Eunospic",spnames)],
V(redl)$name[grep("Scoljoub",spnames)], # "Scolymasta joubini" <- "Scolymastra joubini" 
V(redl)$name[grep("Clavfran",spnames)],
V(redl)$name[grep("Mycaacer",spnames)],
V(redl)$name[grep("Troplong",spnames)],
V(redl)$name[grep("Iphicycl",spnames)], # Espacio en el nombre
V(redl)$name[grep("Dolllong",spnames)],
V(redl)$name[grep("Galiglac",spnames)],
V(redl)$name[grep("Moroinge",spnames)],
V(redl)$name[grep("Calaacut",spnames)],
V(redl)$name[grep("Vibianta",spnames)],
V(redl)$name[grep("Vibisteb",spnames)],
V(redl)$name[grep("Gymnnich",spnames)],
V(redl)$name[grep("Cliopyra",spnames)],
V(redl)$name[grep("Vibianta",spnames)],
V(redl)$name[grep("Vibisteb",spnames)],
V(redl)$name[grep("Gymnnich",spnames)]),ncol=2,byrow=T)
...
require(NetIndices)
troph.net2<-TrophInd(get.adjacency(redl,sparse=F),Dead="Detritus")
layout.matrix.1<-matrix(
  nrow=length(V(redl)),  # Rows equal to the number of vertices
  ncol=2
)
...
ngroups <- length(modulos$csize[modulos$csize>=10])
modulos<-cluster_spinglass(redl,spins=ngroups)


websTbl <- bind_rows(websTbl, data.frame(calc_topological_indices(redl) %>% mutate(Network="Meta",Groups=max(modulos$membership),Modularity=modulos$modularity)))
#
# Make short names to check duplicates!!!!!!!!!! 
#
require(vegan)
