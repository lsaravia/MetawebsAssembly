...
```{r ModularityAnalysisRegional, eval=F,echo=F,message=T,warning=T}
...
saveRDS(tbl,"tbl.rds")
#tbl <- readRDS("tbl.rds")
websTbl1 <- websTbl1 %>% bind_cols(tbl$su)
...
websTbl$zMO[2] <- with(websTbl[2,], (Modularity - rndMO)/sd(sim$modularity))
...
websTbl$zMO[3] <- with(websTbl[3,], (Modularity - rndMO)/sd(sim$modularity))
...
# Check Krill
#
E(redl)[from(V(redl)["Euphausia superba"])]
dtot %>% filter(grepl("Euphausia superba",Prey_name))
...
## Make a map of antarctica weddell sea and Potter Cove

<http://eriqande.github.io/rep-res-web/lectures/making-maps-with-R.html#map-making-in-R>
<https://rpubs.com/nickbearman/r-google-map-making>
<http://www.polarmicrobes.org/making-maps-in-r/>
...
```{r AntarcitaWeddelPotterMap, eval=T,echo=F,message=T,warning=T}

# bajé http://www.naturalearthdata.com/http//www.naturalearthdata.com/download/50m/raster/NE1_50M_SR_W.zip
...
# gdal_translate -r lanczos -ts 1000 0  -co COMPRESS=LZW NE1_50M_SR_W_sh60_polarstereo.tif NE1_50M_SR_W_sh60_polarstereo_small.tif

require(ggmap)

map1 <- get_map(location = "King George Island", maptype = "hybrid", source = "google")
map1 <- get_map(location = "Antarctica", maptype = "hybrid", source = "google")
