...
# Check the species that are hub connectors degree and trophic level
...
topoRolesDegree <- getTopoRolesTLdegree(dtot,"Local",c("Fresh detritus","Aged detritus","Necromass"),hub_conn,"hubcon")
topoRolesDegree <- bind_rows(topoRolesDegree, getTopoRolesTLdegree(dtot,"Regional",c("Detritus","Necromass"),hub_conn,"hubcon"))
topoRolesDegree <- bind_rows(topoRolesDegree, getTopoRolesTLdegree(dtot,"Meta",c("Detritus","Necromass"),hub_conn,"hubcon"))

topoRolesDegree <- bind_rows(topoRolesDegree, getTopoRolesTLdegree(dtot,"Meta",c("Detritus","Necromass"),hub_conn,"modspe"))

topoRolesDegree %>% filter(type=="modspe", preys<2,predators<2)

require(pander)
panderOptions('table.split.table',Inf)
panderOptions('table.style', 'multiline')

pandoc.table(topoRolesDegree %>% filter(type=="hubcon") %>% dplyr::select(Network,name,preys,predators,trophLevel) %>% mutate_if(is.numeric, round, digits=2))
