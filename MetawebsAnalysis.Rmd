...
pandoc.table()
pandoc.table(t(websTbl %>% dplyr::select(Network,Size,Links,Connectance,PathLength,Clustering,Modularity,Groups,rndCC:GRhigh) %>% mutate_if(is.numeric, round, digits=4)))
...
motif
require(tidyr)
!str_detect(names(motif),"Network")
motif <- motif %>% dplyr::select(Network, 1:length(names(motif)))

gather(Network, val, 2:ncol(motif)) %>%
spread_(names(motif)[1], val)
```
