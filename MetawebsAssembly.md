...
More theoretical studies like @Galiana2018 focuses on network-area relationships (NAR) as an extension of species-area relationships (SAR), they assume implicitly that local stability have no influence in NARs, but they hypothesize that stability would change due to the assembly process with area. 

Much work was devoted to identifying the special properties that confer food web stability [@May1972; @McCann2000a], which in fact measure the ability to persist in time [@Grimm1992]. Therefore we expect that particular structural properties that confer stability will be over-represented in real food webs [@Borrelli2015]. Some theorists conceive of assembly as a a non-Darwinian selection process [@Borrelli2015a], whereby species and structures that destabilise the web will be lost and stabilising structures persist [@Prill2005; @Pawar2009; @Borrelli2015a ]. Typically, assembly simulations produce large webs that are both stable in the dynamical sense and relatively resistant to further invasions [@Luh1993; @Law1996; @Drake1990]. 

In this study we analysed the assembly process utilizing a probabilistic model that simulates colonization from a metaweb, local extinctions, and secondary extinctions when predators do not have preys to persist locally (Figure 1), this model lacks any restriction related to dynamical stability and local habitats. We compare structural network properties, including global ones like modularity and mean trophic leve, sub-network properties like motifs, and topological roles that estimate the role of species regarding the network modular structure. We used three metawebs with a total of 57 local food web, and the assembly model simulations to contrast with real local food webs.
...
We selected properties related to stability and resilience like:  trophic coherence, based on the distances between the trophic positions of species and measures how well species fall into discrete trophic levels [@Johnson2014;@Johnson2017]; mean trophic level, historically used as an ecosystem health indicator [@Pauly1998], predicting that food webs with higher trophic levels are less stable [@Borrelli2014]; modularity, that measures the existence of groups of prey and predators that interact more strongly with each other than with species belonging to other modules and prevent the propagation of perturbations throughout the network, increasing its persistence [@Stouffer2011], and a measure of the local stability using the eingenvalues of randomly parametrised networks. 
