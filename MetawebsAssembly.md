...
Recently, there has been an increase in food web assembly studies, integrating them with island biogeography [@Gravel2011; @Galiana2018], metacommunity dynamics [@Pillai2011; @Liao2016] and the effects of habitat fragmentation [@Mougi2016]. 

Very few studies have analysed the process of food web assembly using experimental or empirical data:  @Piechnik2008 found that the first to colonize are trophic generalists followed by specialist, supporting the hypothesis that biotic interactions are important in the assembly process [@Holt1999]; @Baiser2013 showed that habitat characteristics and dispersal capabilities were the main drivers of the assembly and @Fahimipour2014 found that colonization rates were also an important factor. None of them focuses on topological network properties that could be related to different assembly processes.

We selected properties related to stability and resilience like: small-worldness, associated with rapid responses to disturbances and resistance to secondary extinctions [@Montoya2002]; trophic coherence, based on the distances between the trophic positions of species and measures how well species fall into discrete trophic levels [@Johnson2014;@Johnson2017]; mean trophic level, historically used as an ecosystem health indicator [@Pauly1998], predicting that food webs with higher trophic levels are less stable [@Borrelli2014]; modularity, that measures the existence of groups of prey and predators that interact more strongly with each other than with species belonging to other modules and prevent the propagation of perturbations throughout the network, increasing its persistence [@Stouffer2011]. 

To relate detect the process of habitat filtering or dispersal limitation in local food webs we calculate topological roles, these characterize how many trophic links are conducted within their module and/or between modules [@Guimera2005; @Kortsch2015]. Theoretical and empirical results suggest these roles are related to species traits, such as niche breadth, environmental tolerance, apex position in local communities and motility [@Borthagaray2014;@Guimera2010;@Rezende2009;@Kortsch2015;@Dupont2009]. 

<!---Motifs -->
Finally we consider the abundance sub-networks that deviates significantly from a null model network, which are called motifs [@Milo2002]. We analyse here the four three-species motifs that have been most studied theoretically and empirically in food webs [@Prill2005;@Stouffer2007;@Baiser2016] (Figure S5). During the assembly process, motifs that are less dynamically stable tend to disappear from the food web [@Borrelli2015a; @Borrelli2015]. Furthermore, different motifs patterns could also be the result of habitat filtering [@Baldassano2016; @Dekel2005] 
...
We first estimated if the topological network metrics, from local food webs, are different from a random network model, that implies the absence of any ecological mechanism; after that, we compared them with the results of the assembly model. The model simulates the process of colonization from a metaweb and local extinction, with the addition of the network structure and the restriction that predators need prey to survive. Then based in 1000 simulations we calculated z-scores and 99% confidence intervals for each of the properties based on the empirical networks, this includes the metaweb and the local food webs.

All networks presented the small-world topology as their small-world-ness index was larger than the 99% confidence interval (Table 1 & S1). As expected the other metrics are also different from the random model: trophic coherence are lower indicating more stability, modularity is higher except for Potter Cove. 

However, we did not find differences between the local food webs and the assembly model (Figure 2 & Table S2), except for the Weddell Sea food web, meaning that it is more stable than the networks generated by the model. Overall, networks differed from the random null model though presented similarities with the metaweb assembly model.
...
![Network topological properties compared with the metaweb assembly model for A) Potter Cove and B) Weddell Sea food webs. The properties were: Small-world-ness (SWness), coherence (Q), mean trophic level (mTl) and modularity. We ran 1000 simulations of local networks fitted to Potter Cove and Weddell Sea food webs to build the 99% confidence intervals of a particular property (vertical blue lines) and compare with the empirical calculated value (Black arrows). ](Figures/globProp_assembly_ByNetwork.png){ width=100% }

Then we compared motif representation, the four motifs that we used are: exploitative competition, where two predators consume the same pray; apparent competition, where two preys share a predator; omnivory, where we have two predators that consume the same prey but one predator also consumes the other and a tri-trophic chain, where the top predator consumes an intermediate predator that consumes a basal prey (Figure S5). The three-species motifs concerning the random model showed similar representation patterns for all networks (Figure 3A, Table S3): exploitative competition, apparent competition, and omnivory were over-represented, tri-trophic chains were under-represented; all these patterns were significant. To test if motifs proportions are independent of the spatial scale we made a Chi-squared test (Chi-squared = 12612, p-value < 1e-04), that showed that local networks are not a sample of the metaweb. 

Comparing the motifs generated from the metaweb assembly model only some of them were different to the empirical local food webs (Figure 3B, Table S4): tri-trophic chains and omnivory were under-represented for the Weddell Sea, and apparent competition was over-represented for Potter Cove (Figure 3B). From this analysis, we hypothesize that different factors influence each of the local networks. 
...
Finally we looked at topological roles, that measures how species links are distributed between the modules obtained with an optimization algorithm: species that connect all modules are *hub connectors*; species that have a low number of links mostly between modules are *module connectors*; species that have dense connections inside its module are *module hubs* and *module specialists* have low number of links inside its module. We observed that the topological roles frequencies were different among networks (Chi-squared = 79.31, p-value = 1e-04). A higher presence of module connectors (few links, mostly between modules) was observed in the Weddell Sea, while a lack of module hubs (high number of links inside its module) was found in Potter Cove (Figure 3A), which can be related to its low modularity value (Table 1). Then we generated local networks from the metaweb using the assembly model and the topological role frequencies were not different (Chi-squared = 5.95, p-value = 0.41)(Figura 3B). This suggests that local networks species are filtered by habitat or by dispersal limitation.  
